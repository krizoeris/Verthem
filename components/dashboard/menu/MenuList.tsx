    ...
    return (
        <ul className="w-full flex flex-col gap-3 justify-center items-center">
            {menuList.map((menu, index) => {
                ...
                return (
                <li key={index} className={`${pathName === `${menu.link}` ? 'w-full group py-2 bg-slate-100 rounded-md' : 'w-full group py-2 group hover:bg-slate-100 hover:transition-all hover:ease-in rounded-md'}`}>
                    <Link href={menu.link}>
